...
def test_configure_argument_parser(action, option_string, dest, choises, help_str):
    got = configs.configure_argument_parser(choises)
    got_actions = [g for g in got._actions if isinstance(g, action) and g.dest == dest]
    ...
    assert got_action.dest == dest, f"Укажите имя атрибута для {got_action.help}"
    assert (
